 ...
 */
export function fitHabitsInSchedule(habits, availability) {
    const warnings = [];
    const remaining = { ...availability }; // mutable copy
    const fitted = habits.map((habit) => {
 ...
 */
export function buildScheduleSummary(schedule, fittedHabits, excludeCommitmentId) {
    const waking = schedule.wakingMinutesPerDay ?? DEFAULT_WAKING_MINUTES;
    // Compute total committed minutes per day (without new habits)
    const committedMap = ALL_DAYS.reduce((acc, day) => ({ ...acc, [day]: 0 }), {});
    for (const bc of schedule.baseCommitments) {
        if (excludeCommitmentId && bc.id === excludeCommitmentId)
            continue;
        const totalMins = bc.minutesPerDay + (bc.commuteMinutes || 0);
        for (const day of bc.scheduledDays)
            committedMap[day] += totalMins;
    }
    for (const task of schedule.dayTasks) {
        if (!task.recurringDays)
            continue;
        for (const day of task.recurringDays)
            committedMap[day] += task.estimatedMinutes;
    }
    for (const habit of schedule.committedHabits) {
        if (!habit.isActive)
            continue;
        for (const day of habit.scheduledDays)
            committedMap[day] += habit.dailyMinutes;
    }
    // Compute new habit minutes per day
    const newMap = ALL_DAYS.reduce((acc, day) => ({ ...acc, [day]: 0 }), {});
 ...
 */
function assignDays(habit, targetCount, remaining, warnings) {
    ...
    const sortedByAvailability = [...candidateDays].sort((a, b) => remaining[b] - remaining[a]);
    // Pick the top N days that can actually fit the habit
    const fittingDays = sortedByAvailability.filter((day) => remaining[day] >= habit.dailyMinutes);
    if (fittingDays.length === 0) {
        // Nothing fits at all — take the best day anyway and mark as overloaded
        warnings.push(`⚠️ "${habit.name}" (${habit.dailyMinutes} min) no encaja en ningún día disponible. ` +
    ...
    if (fittingDays.length < targetCount) {
        warnings.push(`ℹ️ "${habit.name}" se planificó para ${fittingDays.length} días en lugar de ${targetCount} ` +
 ...
 */
function suggestTimeSlot(assignedDays, availability) {
    if (assignedDays.length === 0)
        return undefined;
    const isWeekendOnly = assignedDays.every((d) => d === 'saturday' || d === 'sunday');
    const isWeekdayOnly = assignedDays.every((d) => d !== 'saturday' && d !== 'sunday');
    if (isWeekendOnly)
        return 'fin de semana, en cualquier momento del día';
    if (isWeekdayOnly)
        return 'entre semana, antes o después del trabajo';
 ...
 */
export function buildEmptySchedule(userId) {
    return {
        userId,
        baseCommitments: [
            {
