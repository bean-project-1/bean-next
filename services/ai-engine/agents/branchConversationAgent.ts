...
import {
  BRANCH_CONVERSATION_SYSTEM_PROMPT,
  BRANCH_CONVERSATION_EXTRACT_INTENT_PROMPT,
  BRANCH_CONVERSATION_PRESENT_PLAN_PROMPT,
} from '../prompts/systemPrompts.js';

// ─── Types ────────────────────────────────────────────────────────────

export interface ConversationTurn {
  /** The agent's reply to display to the user */
  agentReply: string;
  /** Updated conversation state to persist in DB */
  updatedState: BranchConversationState;
  /** Set when the stage reaches 'confirmed' — ready to create the branch */
  confirmedPlan?: BranchPlan;
}

/** Minimal DB adapter interface — inject your actual DB client */
export interface ConversationRepository {
  findByUserId(userId: string): Promise<BranchConversationState | null>;
  save(state: BranchConversationState): Promise<BranchConversationState>;
}

/** Minimal schedule fetcher — inject your actual DB query */
export interface ScheduleRepository {
  findByUserId(userId: string): Promise<GlobalSchedule | null>;
}
 ...
 */
function hasEnoughInfoForIntent(state: BranchConversationState): boolean {
  ...
  const hasTimeReference =
    /semana|mes|año|week|month|year|\d+ días|\d+ weeks|\d+ months/i.test(userMessages);
 ...
 */
function isConfirming(message: string): boolean {
  return /\b(sí|si|yes|confirmo|confirmar|apruebo|adelante|perfecto|listo|ok|okay|va)\b/i.test(
    message
  );
}
 ...
 */
function isRequestingAdjustment(message: string): boolean {
  return /\b(cambiar|ajustar|modificar|change|adjust|modify|menos|más|reduce|increase|diferente)\b/i.test(
    message
  );
}
...
export async function processMessage(
  ...
  buildPlanFn: (intent: BranchIntent, schedule: GlobalSchedule) => Promise<BranchPlan>
  ...
  let confirmedPlan: BranchPlan | undefined;
  ...
  switch (state.stage) {
    ...
    case 'clarifying_constraints': {
      ...
      if (hasEnoughInfoForIntent(state)) {
        ...
        const { reply, nextStage, extractedIntent } = await handleIntentExtraction(
          state,
          client
        );
        agentReply = reply;
        state.stage = nextStage;
        if (extractedIntent) state.extractedIntent = extractedIntent;
      } else {
    ...
    case 'confirming_intent': {
      if (isConfirming(userMessage)) {
        ...
        const { reply, plan, scheduleWasInferred } = await handlePlanGeneration(
          state,
          userId,
          scheduleRepo,
          buildPlanFn,
    ...
    case 'adjusting_plan': {
      ...
      const { reply, plan, scheduleWasInferred } = await handlePlanGeneration(
        ...
        buildPlanFn,
        client,
        userMessage // pass the adjustment note
      );
      agentReply = reply;
      if (plan) {
        state.currentPlan = plan;
 ...
 */
async function handleIntentExtraction(
  state: BranchConversationState,
  client: OpenAI
): Promise<{
  reply: string;
  nextStage: ConversationStage;
  extractedIntent: BranchIntent | null;
}> {
  ...
  try {
    ...
    if (!intentJson) throw new Error('Empty intent extraction response');

    const extractedIntent = JSON.parse(intentJson) as BranchIntent;
    ...
    const confirmationCompletion = await client.chat.completions.create({
      ...
      messages: [
        ...
        ...formatMessagesForAPI(state.messages),
        {
          role: 'user',
          content:
            `He extraído la siguiente intención: ${intentJson}\n\n` +
...
): Promise<{ reply: string; plan: BranchPlan | null; scheduleWasInferred: boolean }> {
  ...
  try {
    ...
    const presentationCompletion = await client.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: BRANCH_CONVERSATION_PRESENT_PLAN_PROMPT(
    ...
    const reply =
      presentationCompletion.choices[0]?.message?.content ??
 ...
 */
async function generateAdjustmentReply(
  state: BranchConversationState,
  ...
  const completion = await client.chat.completions.create({
    ...
    messages: [
      ...
      ...formatMessagesForAPI(state.messages),
      {
        role: 'user',
        content:
          `El usuario quiere ajustar el plan con: "${adjustmentRequest}"\n` +
...
async function generateConversationalReply(
  ...
  const stageHints: Record<ConversationStage, string> = {
    ...
    clarifying_schedule:
      'Pregunta sobre el tiempo disponible del usuario: ¿cuántas horas al día puede dedicar y qué días?',
    clarifying_constraints:
      'Pregunta si hay restricciones: presupuesto, condición física, horarios fijos, etc.',
...
function createInitialState(userId: string): BranchConversationState {
  return {
    id: randomUUID(),
    userId,
    stage: 'greeting',
...
function formatHistoryForPrompt(messages: ConversationMessage[]): string {
  return messages
...
function buildConfirmationMessage(plan: BranchPlan): string {
  const habitList = plan.habits
    .map((h) => `• ${h.name} (${h.dailyMinutes} min, ${h.scheduledDays.join(', ')})`)
    .join('\n');

  return (
    `🌱 ¡Perfecto! Tu nueva rama "${plan.title}" está confirmada.\n\n` +
 ...
 */
function buildMinimalScheduleFromConversation(
  state: BranchConversationState,
  userId: string
): GlobalSchedule {
    ...
    baseCommitments: [
      {
        ...
        scheduledDays: ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as Array<'monday' | 'tuesday' | 'wednesday' | 'thursday' | 'friday' | 'saturday' | 'sunday'>,
        isReplaceable: false,
      },
      ...(hasWork
        ? [
            {
              ...
              minutesPerDay: 480, // 8 hours
              scheduledDays: ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'] as Array<'monday' | 'tuesday' | 'wednesday' | 'thursday' | 'friday' | 'saturday' | 'sunday'>,
              isReplaceable: false,
            },
          ]
        : []),
