...
import { X, RefreshCw, MessageSquare, ChevronRight, ChevronDown, Trash2 } from 'lucide-react';
...
export function GlobalAIChat({ isOpen, onClose, initialMessage, context = 'global', existingGoalData }: GlobalAIChatProps) {
  ...
  const [itemToDelete, setItemToDelete] = useState<{type: string, pIdx: number, tIdx?: number, sIdx?: number} | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex justify-center sm:justify-end bg-stone-900/40 backdrop-blur-sm p-2 sm:p-6" onClick={onClose}>
      ...
      >
        {/* GLOBAL HEADER */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100 bg-stone-50/50 shrink-0 z-20">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white shadow-sm shrink-0">
            ...
            <button 
              type="button" 
              onClick={handleReset} 
              className="ml-1 p-1.5 text-stone-400 hover:text-stone-700 hover:bg-stone-200 rounded-full transition-colors flex items-center justify-center" 
          ...
          {(draftPlan || isDrafting) && (
            <div className="flex md:hidden bg-stone-200/50 p-1 rounded-xl shrink-0">
              <button onClick={() => setMobileTab('chat')} className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${mobileTab === 'chat' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>💬 Chat</button>
              <button onClick={() => setMobileTab('draft')} className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${mobileTab === 'draft' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>📝 Borrador</button>
            </div>
          )}
