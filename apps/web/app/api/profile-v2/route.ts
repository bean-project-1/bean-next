...
// Use the LOCALLY generated client to bypass cache
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 2. Fetch Attributes manually
    const attributes = await (prisma as any).userAttribute.findMany({
      where: { userId: user.id },
      include: { dimension: true }
    });

    // 3. Fetch Latest LifeState manually
    ...
    const latestStateRaw = lifeStates[0] ?? null;
    ...
    const latestState = latestStateRaw ? {
      ...latestStateRaw,
      scores: latestStateRaw.scores.map((s: any) => ({
        ...s,
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[GET /api/profile-v2]', msg);
    return NextResponse.json(
      { 
        success: false, 
