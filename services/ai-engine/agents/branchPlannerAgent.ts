...
import type { BranchIntent, BranchPlan, GlobalSchedule } from '@bean/types';
import {
  BRANCH_PLANNER_SYSTEM_PROMPT,
  BRANCH_PLANNER_BUILD_PLAN_PROMPT,
...
function getDeepSeekClient(): OpenAI {
  ...
  if (!apiKey) {
    throw new Error(
      'DEEPSEEK_API_KEY is not set. Add it to your .env file: DEEPSEEK_API_KEY=sk-...'
    );
  }
  return new OpenAI({
    apiKey,
...
export async function buildPlan(
  ...
  const plannerPrompt = BRANCH_PLANNER_BUILD_PLAN_PROMPT(
    JSON.stringify(intent, null, 2),
    JSON.stringify(schedule, null, 2),
    wakingMinutes
  );

  let rawPlan: BranchPlan;
  ...
  try {
    ...
    if (!content) throw new Error('Empty response from DeepSeek planner');

    rawPlan = JSON.parse(content) as BranchPlan;
  } catch (error) {
    console.error('[BranchPlannerAgent] DeepSeek call failed:', error);
    throw new Error(
      `BranchPlannerAgent failed to generate a plan: ${(error as Error).message}`
    );
  }
  ...
  const { fitted: fittedHabits, warnings: fitWarnings } = fitHabitsInSchedule(
    rawPlan.habits,
    availability
  );
  ...
  const isSustainable = !Object.values(scheduleSummary.byDay).some(
    (day) => day.isOverloaded
  );

  if (!isSustainable && !allWarnings.some((w) => w.includes('sobrecarg'))) {
    allWarnings.push(
      '⚠️ El plan supera el tiempo disponible en algún día de la semana. ' +
